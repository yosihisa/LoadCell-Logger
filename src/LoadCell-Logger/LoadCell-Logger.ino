  ...
  switch (state) {
    case READ:
      ...
      if ((read[0] != 0) & (read[1] != 0) & (read[2] != 0) & (read[3] != 0)) {
        ...
        digitalWrite(LED1, read[0] == 1);
        digitalWrite(LED2, read[1] == 1);
        digitalWrite(LED3, read[2] == 1);
        digitalWrite(LED4, read[3] == 1);

        uint8_t tx_state = (counter % 100) < 50 ? 1 : 0;
        digitalWrite(PIN_TX, tx_state);
        digitalWrite(PIN_LED, tx_state);

        Serial.printf("%+8d,%+8d,%+8d,%+8d,  %+12.5f,%+12.5f,%+12.5f,%+12.5f,  %d,%d,%d,%d, %c\n",
